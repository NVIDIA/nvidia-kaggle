Reading kernel: romantamrazov/rogii-super-solution-lb-top-3
Cells: 11 (10 code, 1 markdown) | 36368 chars

┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃                   ROGII — Sub-9 Solution v2 (Tuned LGB+CB)                   ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛

From 10.184 → target < 9.0 | ~55 min on T4×2                                    

             Strategy: polish the proven pipeline, don't add noise              

 • LGB: num_leaves=255, lr=0.025, 8000 iter + better regularisation             
 • CB: depth=7, 8000 iter, lr=0.025, border_count=254 (GPU quality)             
 • Numba JIT beam ±2 delta, 7 configs                                           
 • Multi-scale NCC self-corr (hw=8,15,25) → 3 TVT signals                       
 • WLS b_well (recent rows upweighted, decay=0.02)                              
 • Formation consensus std/range → uncertainty features                         
 • Per-formation known-zone RMSE → which surface to trust                       
 • Inter-signal std → master uncertainty feature                                
 • GR envelope + energy (rolling max / RMS)                                     
 • Richer tw_diff: 4th family anchored at PF-ANCC                               
 • Monotonic GR trend (linear detrended residual)                               
 • Smart ridge stack: 4 models, positive=True                                   
...
────────────────────────────────────────────────────────────────────────────────
 ...
 for pkg in ["numba"]:                                                          
     if                                                                         
 subprocess.run([sys.executable,"-m","pip","show",pkg],capture_output=True).ret 
 ncode!=0:                                                                      
...
────────────────────────────────────────────────────────────────────────────────
 ...
 def _find():                                                                   
     for p in [Path("/kaggle/input/rogii-wellbore-geology-prediction"),         
                Path("/kaggle/input/competitions/rogii-wellbore-geology-predict 
 n")]:                                                                          
         if (p/"train").exists(): return p                                      
     for p in Path("/kaggle/input").glob("*/sample_submission.csv"): return     
 p.parent                                                                       
 ...
 print(f"CPUs={NCPU} |                                                          
 wells={len(list(TRAIN_DIR.glob('*__horizontal_well.csv')))}")                  
...
────────────────────────────────────────────────────────────────────────────────
 ...
 def _smooth(v,fb,r):                                                           
     s=pd.Series(v,dtype='float32').interpolate(limit_direction='both').fillna( 
 )                                                                              
 ...
 def beam_search(gr_h,tw_tvt,tw_gr,start_tvt,bs=10,mc=20.,es=144.,r=2):         
     ...
     return                                                                     
 tw_tvt[_beam_jit(sgr,tw_gr.astype(np.float64),si,bs,float(mc),float(es))].asty 
 (np.float32)                                                                   
...
────────────────────────────────────────────────────────────────────────────────
 ...
 def run_pf_ancc(hw,tw_tvt,tw_gr,N=ANCC_N):                                     
     ...
     for i in range(len(ev)):                                                   
         ...
         ne=1./np.sum(w**2)                                                     
         if ne<PF_RESAMP*N:                                                     
             ix=np.searchsorted(np.cumsum(w),(np.arange(N)+np.random.uniform()) 
 )                                                                              
 ...
 lues                                                                           
     tf_p=interp1d(tw_tvt,tw_gr,bounds_error=False,fill_value=(tw_gr[0],tw_gr[- 
 ))                                                                             
     tf_s=interp1d(tw_tvt,tw_s, bounds_error=False,fill_value=(tw_s[0],         
 tw_s[-1]))                                                                     
...
────────────────────────────────────────────────────────────────────────────────
 ...
 print(f"  FPK: {len(FI.df)} centroids | Dense: {len(DI.ancc):,} pts            
 ({time.time()-t0:.0f}s)")                                                      
...
────────────────────────────────────────────────────────────────────────────────
 ...
 oat(np.nanmean(tw_gr)))                                                        
     ...
     for fi2,fn in enumerate(FORMATIONS):                                       
         ...
         form_rmse[fn]=float(np.sqrt(np.mean((ktvt-(-z_kn+form_kn[:,fi2]+b_all) 
 *2)))                                                                          
 ...
 bpaths.values()],1),1).astype(np.float32),                                     
         # Multi-scale NCC                                                      
         'sc8_d' :(sc8 -np.float32(last_tvt)).astype(np.float32),'sc8_score'    
 :sc8s,                                                                         
         'sc15_d':(sc15-np.float32(last_tvt)).astype(np.float32),'sc15_score':s 
 5s,                                                                            
         'sc25_d':(sc25-np.float32(last_tvt)).astype(np.float32),'sc25_score':s 
 5s,                                                                            
 ...
 .float32),                                                                     
         ...
         **{f'tvtF_{fn}_d':(tvt_fs[fn]-last_tvt).astype(np.float32)      for fn 
 in FORMATIONS},                                                                
         **{f'tvtFw_{fn}_d':(tvt_fs[fn+'_wls']-last_tvt).astype(np.float32) for 
 fn in FORMATIONS},                                                             
 ...
 t32)  for o in BEAM_OFFS},                                                     
         **{f'tdsc{int(o)}':hgr-np.interp(sc15+o,tw_tvt,tw_gr).astype(np.float3 
 for o in SC_OFFS},                                                             
         **{f'tdpf{int(o)}':hgr-np.interp(pf_use+o,tw_tvt,tw_gr).astype(np.floa 
 2)    for o in PF_OFFS},  # NEW 4th family                                     
...
────────────────────────────────────────────────────────────────────────────────
 ...
 def run_lgb(cfg_idx):                                                          
     ...
     oof=np.zeros(len(train_df),np.float32); tp=np.zeros(len(test_df),np.float3 
     for fold,(tr,va) in enumerate(splits):                                     
         ...
         m=lgb.train(p,dtr,valid_sets=[dva],num_boost_round=n_est,              
                     callbacks=[lgb.early_stopping(250,verbose=False),lgb.log_e 
 luation(800)])                                                                 
         oof[va]=m.predict(X.iloc[va],num_iteration=m.best_iteration).astype(np 
 loat32)                                                                        
         tp+=m.predict(Xt,num_iteration=m.best_iteration).astype(np.float32)/N_ 
 LITS                                                                           
         print(f"  LGB{cfg_idx} f{fold}:                                        
 rmse={root_mean_squared_error(y.iloc[va],oof[va]):.4f}"                        
...
────────────────────────────────────────────────────────────────────────────────
 ...
 print(f"Best: alpha={best_cfg[0]:.2f} tau={best_cfg[1]} w_pf={best_cfg[2]:.2f} 
 abs TVT RMSE={best_r:.4f}")                                                    
 ...
 print(f"\n✅  {OUT}  {len(sub)} rows")                                         
 print("\n─── Final Summary ─────────────────────────")                         
 for k,v in results.items(): print(f"  {k:8s}: OOF residual RMSE =              
 {v['rmse']:.4f}")                                                              
